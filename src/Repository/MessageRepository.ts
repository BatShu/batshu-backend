...
export const insertMessageRow = async (connection: PoolConnection, messageObject: SendMessageRequest): Promise<boolean> => {
    try{
        ...
        await connection.execute(insertQuery,[messageObject.sendUserUid, messageObject.roodId, messageObject.message])
        return true;
    } catch(err){
        return false;
    }
...
export const selectMessageRow = async (connection: PoolConnection, roomId: number): Promise<SelectMessageRow[]> => {
    try{
        const selectQuery = `SELECT uid, message_text, created_at FROM message where room_id = ?`;
...
export const selectRoomRow = async (connection: PoolConnection, roomId: number): Promise<SelectRoomRow> => {
    try{
        const selectQuery = `SELECT uid, report_uid, accidentId, observeId FROM room where room_id = ?`;
